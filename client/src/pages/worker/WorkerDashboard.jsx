...
import { fetchRecentActivity } from '../../store/analyticsSlice';

const WorkerDashboard = () => {
  const dispatch = useDispatch();
  const { user } = useSelector((state) => state.auth);
  const { recentActivity } = useSelector((state) => state.analytics);
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
              ...
              <div>
                <QuickActions actions={workerActions} />
              </div>
            </div>
