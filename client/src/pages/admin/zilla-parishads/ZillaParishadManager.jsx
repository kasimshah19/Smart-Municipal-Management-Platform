...
function ZillaParishadManager() {
  ...
  }, [page, limit, districtFilter, statusFilter, appliedSearch, appliedLgdCode]);
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await api.get(`/zilla-parishads${queryParams}`);
      ...
      dispatch(showToast('Failed to fetch Zilla Parishads'));
      console.error(err);
    } finally {
      setLoading(false);
    }
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      // Find districtName from selected districtId
      ...
      if (editingItem) {
        await api.put(`/zilla-parishads/${editingItem._id}`, submitData);
        dispatch(showToast('Zilla Parishad updated successfully'));
      } else {
        await api.post('/zilla-parishads', submitData);
        dispatch(showToast('Zilla Parishad created successfully'));
      }
      setIsModalOpen(false);
      fetchData();
    } catch (err) {
      dispatch(showToast(err.response?.data?.message || 'Failed to save Zilla Parishad'));
    }
  };
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-sm mt-1" style={{ color: 'var(--muted)' }}>
            Manage {totalRecords > 0 ? totalRecords.toLocaleString() : '...'} Zilla Parishads.
          </p>
        </div>
        <button 
        ...
        <div className="overflow-x-auto min-h-[400px]">
          <table className="w-full text-left text-sm">
            ...
            <tbody>
              {loading ? (
                <tr>
                  <td colSpan="5" className="py-12 text-center" style={{ color: 'var(--muted)' }}>
                    Loading Zilla Parishads...
                  </td>
                </tr>
              ) : data.length > 0 ? (
                data.map(item => (
                ...
                <tr>
                  <td colSpan="5" className="py-12 text-center" style={{ color: 'var(--muted)' }}>
                    No Zilla Parishads found matching your criteria.
                  </td>
                </tr>
              )}
            </tbody>
      ...
      <Modal
        ...
        title={editingItem ? 'Edit Zilla Parishad' : 'Add Zilla Parishad'}
        maxWidth="max-w-xl"
      >
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <div className="flex justify-end gap-3 mt-6 pt-4" style={{ borderTop: '1px solid var(--line)' }}>
            ...
            <button
              ...
              style={{ backgroundColor: 'var(--primary)' }}
            >
              {editingItem ? 'Save Changes' : 'Create Zilla Parishad'}
            </button>
          </div>
        </form>
      </Modal>
...
export default ZillaParishadManager;
