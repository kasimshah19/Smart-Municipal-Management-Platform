...
function GramPanchayatManager() {
  ...
  // Trigger search on button click instead of every keystroke for text inputs
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await api.get(`/gram-panchayats${queryParams}`);
      ...
      dispatch(showToast('Failed to fetch Gram Panchayats'));
      console.error(err);
    } finally {
      setLoading(false);
    }
  ...
  const handleOpenModal = (item = null) => {
    if (item) {
      ...
      setFormData({
        name: item.name || '',
        localBodyNameEnglish: item.localBodyNameEnglish || '',
        localBodyNameLocal: item.localBodyNameLocal || '',
        lgdCode: item.lgdCode || '',
        districtId: item.districtId?._id || item.districtId || '',
        talukaId: item.talukaId?._id || item.talukaId || '',
        villageCount: item.villageCount || 1,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingItem) {
        await api.put(`/gram-panchayats/${editingItem._id}`, formData);
        dispatch(showToast('Gram Panchayat updated successfully'));
      } else {
        await api.post('/gram-panchayats', formData);
        dispatch(showToast('Gram Panchayat created successfully'));
      }
      setIsModalOpen(false);
      fetchData();
    } catch (err) {
      dispatch(showToast(err.response?.data?.message || 'Failed to save Gram Panchayat'));
    }
  };
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <div>
          <h2 className="text-xl font-bold" style={{ color: 'var(--ink)' }}>
            Gram Panchayats (Rural Administration)
          </h2>
          <p className="text-sm mt-1" style={{ color: 'var(--muted)' }}>
            Manage {totalRecords > 0 ? totalRecords.toLocaleString() : '...'} rural local bodies.
          </p>
        </div>
        <button 
          ...
          <table className="w-full text-left text-sm">
            ...
            <tbody>
              {loading ? (
                  ...
                  <tr key={item._id} className="group hover:bg-black/5" style={{ borderBottom: '1px solid var(--line)' }}>
                    ...
                    <td className="px-4 py-3 font-medium" style={{ color: 'var(--ink)' }}>
                      ...
                      {item.localBodyNameLocal && (
                        <div style={{ fontSize: '0.7rem', color: 'var(--muted)' }}>{item.localBodyNameLocal}</div>
                      )}
                    </td>
                    ...
                    <td className="px-4 py-3" style={{ color: 'var(--muted)' }}>
                      <div>{item.talukaId ? (talukas.find(t => t._id === item.talukaId)?.name || item.talukaId) : '—'}</div>
                      {item.developmentBlockName && (
                        <div style={{ fontSize: '0.7rem', color: 'var(--muted)' }}>{item.developmentBlockName} (Block)</div>
                      )}
                    </td>
                ...
                <tr>
                  <td colSpan="6" className="py-12 text-center" style={{ color: 'var(--muted)' }}>
                    No Gram Panchayats found matching your criteria.
                  </td>
                </tr>
              )}
            </tbody>
      ...
      <Modal
        ...
        title={editingItem ? 'Edit Gram Panchayat' : 'Add Gram Panchayat'}
        maxWidth="max-w-xl"
      >
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <div className="grid grid-cols-2 gap-4">
            <div className="flex flex-col gap-1">
              ...
              <input
                ...
                className="px-3 py-2 text-[14px] outline-none"
                style={inputStyles}
                disabled={!!editingItem} // LGD code shouldn't change generally, but allow it for new ones
              />
            </div>
            <div className="flex flex-col gap-1">
              ...
              <input
                ...
                required
                value={formData.name}
                onChange={(e) => setFormData({...formData, name: e.target.value, localBodyNameEnglish: e.target.value})}
                className="px-3 py-2 text-[14px] outline-none"
                style={inputStyles}
              />
            </div>
          ...
          <div className="flex flex-col gap-1">
            ...
            <input
              type="text"
              value={formData.localBodyNameLocal}
              onChange={(e) => setFormData({...formData, localBodyNameLocal: e.target.value})}
              className="px-3 py-2 text-[14px] outline-none"
              style={inputStyles}
            />
          </div>
          ...
          <div className="flex flex-col gap-1">
            <label className="text-[13px] font-medium" style={{ color: 'var(--ink)' }}>Village Count</label>
            <input
              type="number"
              min="1"
              value={formData.villageCount}
              onChange={(e) => setFormData({...formData, villageCount: e.target.value})}
              className="px-3 py-2 text-[14px] outline-none w-1/2"
              style={inputStyles}
            />
          </div>
          ...
          <div className="flex justify-end gap-3 mt-6 pt-4" style={{ borderTop: '1px solid var(--line)' }}>
            ...
            <button
              ...
              style={{ backgroundColor: 'var(--primary)' }}
            >
              {editingItem ? 'Save Changes' : 'Create Gram Panchayat'}
            </button>
          </div>
        </form>
      </Modal>
...
export default GramPanchayatManager;
