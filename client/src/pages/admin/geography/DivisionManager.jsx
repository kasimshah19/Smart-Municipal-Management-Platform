...
function DivisionManager() {
  ...
  const fetchData = async () => {
    try {
      ...
      dispatch(showToast('Failed to fetch divisions'));
      console.error(err);
    } finally {
      setLoading(false);
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingItem) {
        await api.put(`/geography/divisions/${editingItem._id}`, formData);
        dispatch(showToast('Division updated successfully'));
      } else {
        await api.post('/geography/divisions', formData);
        dispatch(showToast('Division created successfully'));
      }
      setIsModalOpen(false);
      fetchData();
    } catch (err) {
      dispatch(showToast(err.response?.data?.message || 'Failed to save division'));
    }
  };

  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this division? Check if it has dependent districts first.')) {
      try {
        await api.delete(`/geography/divisions/${id}`);
        dispatch(showToast('Division deleted successfully'));
        fetchData();
      } catch (err) {
        dispatch(showToast(err.response?.data?.message || 'Failed to delete division'));
      }
    }
  };
          ...
          <table className="w-full text-left text-sm">
            ...
            <tbody>
              {filteredData.length > 0 ? (
                ...
                <tr>
                  <td colSpan="4" className="py-8 text-center" style={{ color: 'var(--muted)' }}>
                    No divisions found.
                  </td>
                </tr>
              )}
